...
XTL works with modern C++23 compilers and has been tested with MinGW, GCC, Intel C++, Cygwin and Microsoft Visual C++. The library can be used out-of-the-box in many cases by simply including the desired header since most components are header-only. A few components require linking to a run-time component so they will need to be compiled.

**Note**: XTL targets C++23 and leverages modern C++ features including:
- `std::source_location` for error reporting (with fallback to custom implementation)
- `[[nodiscard]]` attributes for improved API safety
- Modern memory ordering semantics in concurrent primitives
- `std::string::ends_with()` when available (C++20+)
- Replaced deprecated `std::is_pod` with `std::is_trivial_v && std::is_standard_layout_v`

For C++20/23 compilers, the library automatically uses standard library features when available, falling back to custom implementations for backward compatibility.
