...
namespace xtd{
  namespace rfc2229{
    ...
    struct command : _::rule<command, _::and_<cmd_word, _::zero_or_more_< _::and_<WS, cmd_param> >>>{
      template <typename ... _ArgTs> command(_ArgTs&&...oArgs) : rule(std::forward<_ArgTs>(oArgs)...){}
    };

    struct msg_atom : _::rule< msg_atom, _::one_or_more_< _::and_< _::not_<SPACE, CTL, LT, GT, PERIOD, BSLASH>, CHAR>>>{
      template <typename ... _ArgTs> msg_atom(_ArgTs&&...oArgs) : rule(std::forward<_ArgTs>(oArgs)...){}
    };

    struct capabilities : _::rule<capabilities, _::zero_or_one_< _::and_< LT , msg_atom, _::zero_or_more_< _::and_< PERIOD, msg_atom > > , GT >>>{
      template <typename ... _ArgTs> capabilities(_ArgTs&&...oArgs) : rule(std::forward<_ArgTs>(oArgs)...){}
    };
